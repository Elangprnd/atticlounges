        ...
        <div class="bg-white overflow-hidden flex flex-col lg:flex-row">
          <div class="lg:w-2/5 p-6 border-r border-gray-100 flex flex-col">
            <div class="relative w-full pb-[100%] border border-gray-200 rounded-lg overflow-hidden mb-6">
              ...
              <button id="wishlist-btn" class="absolute top-4 right-4 bg-white p-2 rounded-full shadow-md cursor-pointer">
                <svg class="h-6 w-6 ${getWishlist().includes(product._id || product.id) ? 'text-red-500' : 'text-gray-400'}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                </svg>
              </button>
            </div>
            <div class="flex space-x-4">
              <a href="${location.pathname.includes('/pages/') ? 'chat.html' : 'pages/chat.html'}" class="flex-1 flex items-center justify-center gap-2.5 py-3 px-4 border border-transparent rounded-[5px] shadow-md text-lg font-bold bg-[#EDE0D4] hover:bg-[#dcd0c5]">
                <img src="https://cdn-icons-png.flaticon.com/128/2076/2076218.png" alt="" class="w-5 h-5"> Chat
              </a>
              <button id="add-to-cart-btn" class="flex-1 flex items-center justify-center gap-2.5 py-3 px-4 border border-transparent rounded-[5px] shadow-md text-lg font-bold bg-[#EDE0D4] hover:bg-[#dcd0c5]">
                <img src="https://cdn-icons-png.flaticon.com/128/1170/1170678.png" alt="" class="w-5 h-5"> Add to cart
              </button>
            </div>
          </div>
          <div class="lg:w-3/5 p-6">
            ...
            <p id="product-description" class="text-gray-700 mb-6 leading-relaxed">${product.description || `High-quality ${categoryName.toLowerCase()} in excellent condition.`}</p>
            ...
            <div class="bg-gray-50 p-4 rounded-lg grid grid-cols-2 gap-y-2 gap-x-4 text-sm">
              ...
              <div><span class="text-gray-500">Stock :</span> <span id="spec-stock" class="font-medium text-gray-800">${product.stock ?? 1}</span></div>
            </div>
          </div>
        </div>
      </div>
